...
#						<uid>the unique identifier for a aubscribed user</uid>
#						</subscriber>
#					</subscription>
#				<csapi_cobject_data_value>
#
...
sub xmlSetName
{
	...
	$self->{mName} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetValue
{
	...
	$self->{mValue} = substr($xmlData, $iStart, $iEnd - $iStart);
...
sub xmlSetLabel
{
	...
	$self->{mLabel} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetDefault
{
	...
	$self->{mDefault} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetFormattedName
{
	...
	$self->{mFormattedName} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
my $subscription = $apiObject->getSubscription()
 ...
 Example:
	...
	{	
		...
		$problem->getDataObjectByName("subscription1")->setXMLEncodedValue($subscription->toSubmitXml());
...
Sets the "date" property for the class instance.

Note: The setDate property is only used to set a human readable date
that will be used again by the script to display the date.  To submit date
values to the server you must use the B<setValue> method.

$apiObject->setDate($value)
...
Sets the "default value" property for the class instance

$apiObject->setDefault($value)

 Parameters:
	value - the new value for default
...
Sets the "inherited" property for the class instance

$apiObject->setInherited($value)

 Parameters:
	value - the new value for inherited (true or false)
...
Sets the "read only" property for the class instance.

$apiObject->setReadOnly($value)

 Parameters:
	value - the new value for read only (true or false)
...
Sets the "required" property for the class instance.

$apiObject->setRequired($value)

 Parameters:
	value - the new value for required (true or false)
...
The date values must be set in milliseconds since 1970.  PERL dates are
in seconds since 1970.

Example for setting the current time:

  #Get the current time and append three 0's to the end to
...
Example for setting a time in the future or past:
  ...
  $apiObject->setValue($milliseconds);
...
Get XML data used to send to the SYNERGY/Change server.

Used by api functions to construct the XML strings that will be submitted to the
SYNERGY/Change server.

my $xmlData = $apiObject->toAttributeXml()

 Returns: scalar
	the XML data to be submitted to the SYNERGY/Change server.  This function
