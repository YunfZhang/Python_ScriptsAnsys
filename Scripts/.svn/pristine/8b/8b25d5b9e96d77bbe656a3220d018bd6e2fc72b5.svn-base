...
sub parseXml()
{
		...
		($self->{mDataType} eq $self->{globals}->{LIST_TYPE}))
	{
		...
		while($iStart >= 0)
		{
			push @{$self->{mValues}}, substr($xmlData, $iStart, $iEnd - $iStart);
		...
		while($iStart >= 0)
		{
			push @{$self->{mLabels}}, substr($xmlData, $iStart, $iEnd - $iStart);
	...
	if($self->{mDataType} eq $self->{globals}->{REPORT_TYPE})
	{
		...
		if($iStart >= 0)
		{
			...
			if($iEnd >= 0)
			{
				$self->{mQueryName} = substr($xmlData, $iStart, $iEnd - $iStart);
			}
		}
		...
		if($iStart >= 0)
		{
			...
			if($iEnd >= 0)
			{
				$self->{mExportForm} = substr($xmlData, $iStart, $iEnd - $iStart);
			}
		}
	   ...
	   ($self->{mDataType} eq $self->{globals}->{REPORT_TYPE}))
	{
		...
		if($iStart >= 0)
		{
			...
			if($iEnd >= 0)
			{
				$self->{mQueryString} = substr($xmlData, $iStart, $iEnd - $iStart);
			}
		}
...
type data from SYNERGY/Change configuration data. The data that can be obtained
