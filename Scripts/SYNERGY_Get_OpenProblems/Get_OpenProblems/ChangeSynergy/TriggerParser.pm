...
sub parseXml
{
	...
	if(defined($element))
	{
		set_user($self, $element->get_content());
	}
	$element = undef;
	...
	if(defined($element))
	{
		set_admin_user($self, $element->get_content());
	}
	$element = undef;
	...
	if(defined($element))
	{
		set_from_object($self, $element->get_content());
	}
	$element = undef;
	...
	if(defined($element))
	{
		set_from_state($self, $element->get_content());
	}
	$element = undef;
	...
	if(defined($element))
	{
		set_to_state($self, $element->get_content());
	}
	$element = undef;
	...
	if(defined($element))
	{
		set_to_object($self, $element->get_content());
	}
	$element = undef;
	...
	if(defined($element))
	{
		set_relation_name($self, $element->get_content());
	}
	$element = undef;
...
ChangeSynergy::TriggerParser is an XML parser for SYNERGY/Change's input trigger files.
When a trigger is fired, SYNERGY/Change generates an XML file with useful information
...
Get the CM Synergy database to which the user was connected when the trigger fired.
This is useful when creating a new connection to the SYNERGY/Change server.
...
Get the CM Synergy database uid to which the user was connected when the trigger fired.
This is the SYNERGY/Change unique ID of the database.
...
Get the hostname of the SYNERGY/Change installation to which the user was connected when 
the trigger fired. This is useful when creating a new connection to the SYNERGY/Change 
server.

my $host = $trigger->get_host()

 Returns: scalar
	the hostname of the SYNERGY/Change installation which
...
Get the port of the SYNERGY/Change installation to which the user was connected when the
...
my $port = $trigger->get_port()
 ...
 Returns: scalar
	the port of the SYNERGY/Change installation which
...
Get the protocol of the SYNERGY/Change installation to which the user was connected when the
...
my $protocol = $trigger->get_protocol()
 ...
 Returns: scalar
	the protocol type of the SYNERGY/Change installation which
