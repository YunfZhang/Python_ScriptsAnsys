...
sub toSubmitXml()
{
	...
	if($self->{mAction} != $self->{globals}->{UNCHANGED})
	{
		$xmlData .= $self->{globals}->{BGN_SUBSCRIBER};
			...
			$xmlData .= $self->{globals}->{BGN_SUB_UID};
				$xmlData .= $self->{mUid};
			$xmlData .= $self->{globals}->{END_SUB_UID};
...
sub parseXml()
{
	...
	if($@)
	{
		...
		die "Cannot parse subscription data: xmlUid() \n $@";
	}
}
...
sub xmlSetUserName()
{
	...
	$self->{mUserName} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetRealName()
{
	...
	$self->{mRealName} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetEmail()
{
	...
	$self->{mEmail} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetUid()
{
	...
	my $iStart = index($xmlData, $self->{globals}->{BGN_SUB_UID});
	my $iEnd   = index($xmlData, $self->{globals}->{END_SUB_UID});
	...
	$iStart += length($self->{globals}->{BGN_SUB_UID});
	...
	$self->{mUid} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
Get the uid property.  This property is used when updating the XML on the server.

my $xmlData = $subscriber->getUid();

 Returns: scalar
	The uid (unique identifier) of the subscriber represented by this object.
...
Get the XML data to send to the SYNERGY/Change server to update the subscription list.
...
my $xmlData = $subscriber->toSubmitXml();
 ...
 Returns: scalar
	the XML data to be sent to the SYNERGY/Change server.
