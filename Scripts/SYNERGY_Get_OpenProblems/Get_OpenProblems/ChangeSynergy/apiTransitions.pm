...
sub parseXml
{
	...
	$self->{mFromState}		= substr($xmlData, 0, $iDel2);
	$self->{mTransition}	= substr($xmlData, 0, $iDel);
	$self->{mToState}		= substr($xmlData, $iDel2+1, ($iDel - ($iDel2+1)));
	$self->{mName}			= substr($xmlData, $iDel+1);
	...
	if($iPos >= 0)
	{
		$self->{mRelation}	= substr($tmp, $iPos+5);
	}
}
