...
sub new
{
	...
	if(@_ == 4)
	{
		$self->{strUserName}		= shift;
		$self->{strUserPassword}	= shift;
		$self->{strUserRole}		= shift;
		$self->{strUserDatabase}	= shift;
...
A instance of ChangeSynergy::apiUser is created by logging into SYNERGY/Change.
...
Get the user CMSynergy database path property.

my $database= $user->getUserDatabase()

 Returns: scalar
	the CMSynergy database specified in the creation of this apiUser object.
...
my $user = $csapi->Login("u00001", "u00001", "User", "\\\\angler\\ccmdb\\cm_database");
