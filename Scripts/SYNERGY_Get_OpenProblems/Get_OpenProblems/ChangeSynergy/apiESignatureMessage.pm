...
sub xmlSetFullname()
{
	...
	$self->{mFullname} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetPurpose()
{
	...
	$self->{mPurpose} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetComment()
{
	...
	$self->{mComment} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
sub xmlSetAttribute()
{
	...
	$self->{mAttribute} = substr($xmlData, $iStart, $iEnd - $iStart);
}
...
Electronic signatures will allow SYNERGY/Change users to sign change requests
...
but is also applicable to SYNERGY/Change proper.
